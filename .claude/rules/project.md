...
- A Cursor `stop` hook silently runs `mise run lint-fix` when `backend/` or `frontend/` is dirty (no auto-follow-up). Still run lint-fix yourself before finishing if you changed code.
