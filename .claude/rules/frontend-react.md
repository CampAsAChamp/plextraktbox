...
- Named `function` declarations only — `export function Foo(...)`, not `export const Foo = () => ...`
- Named exports only — no `export default`
- Props: `interface FooProps` when non-trivial; inline `{ size?: number }` for tiny icons/badges — never `React.FC` / `FC<Props>`
- Private subcomponents: unexported `function Bar(...)` in the same file
- Skip `memo` / `forwardRef` unless there is a measured need
- Hooks: `export function useFoo()` in `.ts` files
