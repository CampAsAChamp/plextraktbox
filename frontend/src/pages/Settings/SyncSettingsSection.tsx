...
import { type AppSettingsInput, formatExcludeLines, getSettings, parseExcludeLines, updateSettings } from "src/api/settings"
...
const syncSchema = z.object({
  default_cron: z.string().min(1, "Cron expression is required").refine(isValidCronExpression, { message: CRON_INVALID_MESSAGE }),
  cron_timezone: z
    .string()
    .min(1)
    .refine((value) => value === "UTC" || value === "local" || isValidIanaTimezone(value), {
...
export function SyncSettingsSection() {
  ...
  return (
    <Paper id="settings-sync" withBorder p="md" data-settings-section="Sync defaults & safety" style={{ scrollMarginTop: 80 }}>
      <form onSubmit={handleSubmit}>
        <Stack gap="md">
          ...
          <Stack gap="xs">
            ...
            <Text size="sm" c="dimmed">
              How job cron hour/minute fields are interpreted. Separate from display preferences (how timestamps are shown).
            </Text>
            <SegmentedControl
              fullWidth
              value={timezoneMode}
