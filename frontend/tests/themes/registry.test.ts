...
describe("theme registry", () => {
  it("defaults to Cinema Night", () => {
    expect(DEFAULT_THEME_ID).toBe("cinema-night")
    expect(getBuiltinTheme(DEFAULT_THEME_ID)?.name).toBe("Cinema Night")
  })

  it("exposes four built-ins with primary colors", () => {
    expect(BUILTIN_THEMES.map((t) => t.id)).toEqual(["cinema-night", "one-dark-pro", "nord", "dracula"])
