...
describe("hasExpandableContext", () => {
  ...
  it("ignores message when it is the only display field", () => {
    ...
    expect(hasExpandableContext({ message: "Fetching ratings", source: "letterboxd" })).toBe(true)
  })
})
...
describe("logContextForDisplay", () => {
  it("drops message when used as display text", () => {
    expect(logContextForDisplay({ message: "Fetching ratings", source: "letterboxd" })).toEqual({
      source: "letterboxd",
    })
  })
})
